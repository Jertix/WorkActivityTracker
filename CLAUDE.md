...
- **Pulizia automatica EditorHistory** (v5.1): `NumeroMesiMantenimentoHistory` (int, default 3) in `appsettings.json` → `AppConfigService.NumeroMesiMantenimentoHistory`. All'avvio `Home.OnInitializedAsync` lancia in background `PulisciStoricoEditorInBackground()` → `EditorHistoryService.EliminaVecchieAsync(mesi)` (usa `ExecuteDeleteAsync`, EF Core 10). Se elimina righe, mostra il banner `messaggioHistoryPulita` (alert-info in testa al form) che si auto-nasconde dopo 12s. Valore `<= 0` disattiva la pulizia.
- **Duplica "Proseguo l'attività"** (v5.1): `DuplicaAttivita()` NON copia più la Note; imposta `Note = GeneraHeaderProseguo(origine)` = `**** Proseguo l'attività {Id}-{Descrizione}-{NumeroTicket} del {Data:dd/MM/yyyy} ****` (testo plain, reso HTML da `noteEditorHelper.setContent`). `ChangesetCoinvolti` resta copiato. Motivo: evitare di gonfiare il record su lavorazioni multi-giorno.
- **Anteprima "lavoro dei giorni precedenti"** (v5.1): `ProseguoRegex` = `\*{2,}\s*Proseguo l'attività\s+(\d+)`. `NoteContieneProseguo()` decide se mostrare il pulsante (sopra la label Note). `ToggleLavoroPrecedente()` chiama `CostruisciCatenaProseguo()` che segue i puntatori via `ActivityService.GetActivityByIdAsync(id, isAdminMode)` (anti-loop con `HashSet`, limite 500), ordina per `Data` crescente e compone **HTML** (`htmlLavoroPrecedente`) con sezioni `====== ... ======` (HtmlEncode) + la Note in HTML. L'anteprima è un **div di sola lettura con `@((MarkupString)...)`** (classe `.note-preview-readonly`, img vincolate a `max-width:100%` in app.css), NON una textarea: così screenshot e formattazione delle Note precedenti sono visibili. `RimuoviHeaderProseguoHtml()` (regex su HTML, solo se l'header è in testa) toglie la riga "Proseguo" da ogni Note. **Caricamento on-demand con cache per-selezione**: la catena viene letta dal DB solo al primo click (flag `lavoroPrecedenteCaricato`), i "Mostra/Nascondi" successivi riusano `htmlLavoroPrecedente` senza riquery. Stato (`mostraLavoroPrecedente`, `htmlLavoroPrecedente`, `lavoroPrecedenteCaricato`) resettato in `NuovaAttivita`/`SelezionaAttivita`/`DuplicaAttivita` (invalida la cache al cambio attività).
- Versione attuale: **5.1** (aggiornare in `appsettings.json` a ogni release).
